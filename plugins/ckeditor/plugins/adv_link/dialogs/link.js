				...
				// 				http://docs.ckeditor.com/#!/guide/plugin_sdk_sample_1
				{
					...
					children : [
					{
						...
						onLoad : function(element) {
							...
							if(self==top) {
								var pages_url = String(document.location).replace(/\?.*$/, '') + "?action=get_pages_textcontent";
							} else {
								var pages_url = String(parent.document.location).replace(/\?.*$/, '') + "?action=get_ckeditor_pages";
							}
						...
						commit : function( data )
						{
							if ( !data.localPage )
                            	...
                            	//data.localPageName = this._.select.getElement().$.options[this._.select.getElement().$.selectedIndex].text.replace(/^-+/,"");
						}
					}]
				},
				// added by @simo - end
			...
			onOk: function() {
				...
				if ( !this._.selectedElement ) {
					...
					if ( range.collapsed ) {
						...
						if( data.type == 'localPage') var text = new CKEDITOR.dom.text(data.localPageName, editor.document );
						else {
