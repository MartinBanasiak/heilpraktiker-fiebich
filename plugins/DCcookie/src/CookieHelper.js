const COOKIE_EXTERNAL_MEDIA = "external_media";

export class CookieHelper {
    /**
     *
     * @param providers
     * @param managedSiteCode
     * @param lastReset {Date}
     */
    constructor(providers, managedSiteCode, lastReset, siteCode, enableStatistics) {
        this.historyStorage = 'dcCookieHistory';
        this.providerStorage = 'dcCookieProvider';
        this.settingsStorage = 'dcCookieSettings';
        this.lastResetStorage = 'dcCookieLastReset';

        this.siteCode = siteCode;
        this.providers = providers;
        this.enableStatistics = enableStatistics;

        if (managedSiteCode !== '') {
            this.historyStorage += '_' + managedSiteCode;
            this.providerStorage += '_' + managedSiteCode;
            this.settingsStorage += '_' + managedSiteCode;
        }
        this.checkForReset(lastReset);

        this.getDCCookieSettings();
        this.getCookieHistory();
        if(!this.dcCookieSettings) {
            throw "No Settings!";
        }
        if(this.getSettingsAlreadySet()){
            this.getCookieProviderStates();
        }
    }

    /**
     * @param lastReset {Date}
     */
    checkForReset(lastReset) {
        let storage = localStorage;
        if (typeof (storage) !== "undefined") {
            let lastSavedReset = storage.getItem(this.lastResetStorage);
            if (lastSavedReset != lastReset) {
                storage.removeItem(this.historyStorage);
                storage.removeItem(this.providerStorage);
                storage.removeItem(this.settingsStorage);
                storage.removeItem(this.lastResetStorage);
            }
        }
        storage.setItem(this.lastResetStorage, lastReset);
    }

    /**
     * gets the cookie settings from the local storage
     *
     * @returns {boolean}
     */
    getDCCookieSettings(){
        let storage = localStorage;
        var dcCookieSettings;
        var dcCookieSettingsParse;
        if (typeof(storage) !== "undefined" && this.lsTest()) {
            dcCookieSettingsParse = storage.getItem(this.settingsStorage);
            if( dcCookieSettingsParse !== "undefined" && dcCookieSettingsParse !== null){
                if(dcCookieSettings = JSON.parse(dcCookieSettingsParse)){
                    this.dcCookieSettings = dcCookieSettings;
                    return true;
                }
            }
            this.setDCCookieSettings4FirstUser();
            return true;
        }
        /*TODO: lösen*/
        console.log("Kein local Storrage!");
        return false;
    }

    /**
     * Sets the inital settings for the user
     */
    setDCCookieSettings4FirstUser(){
        this.dcCookieSettings = {"dcCookieMember" : this.uuidv4(),"creation_time" : Date(),"settings_already_set":false};
        this.providers.forEach(function (provider) {
            provider.state = false;
        });
        this.setDCCookieSettings();
    }

    /**
     * Sets the dcCookieSettings in the local storage
     */
    setDCCookieSettings(){
        var storage = localStorage;
        storage.setItem(this.settingsStorage,JSON.stringify(this.dcCookieSettings));
    }

    /**
     * tests the local storage, returns true in succeed, otherwise false
     *
     * @returns {boolean}
     */
    lsTest(){
        var test = 'test';
        try {
            localStorage.setItem(test, test);
            localStorage.removeItem(test);
            return true;
        } catch(e) {
            return false;
        }
    }

    /**
     * generates uuid
     *
     * @returns {*}
     */
    uuidv4() {
        return ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, c =>
            (c ^ crypto.getRandomValues(new Uint8Array(1))[0] & 15 >> c / 4).toString(16)
        )
    }

    /**
     * gets already set settings for dcCookieSettings
     *
     * @returns {boolean}
     */
    getSettingsAlreadySet() {
        return this.dcCookieSettings.settings_already_set;
    }

    /**
     * toggles the cookie groups information
     *
     * @param group
     * @param toggle
     */
    toggleCookieGroup(group, toggle){
        this.providers.forEach(function (provider) {
            if(provider.category == group){
                if(provider.state !== toggle){
                    this.doCookieAction(provider,toggle);
                    this.addCookieHistory(provider.id, toggle);
                }
                provider.state = toggle;
            }
        }.bind(this));
        this.setDCCookieProviderStates();
    }

    /**
     * force all cookies to be active
     */
    forceAllCookiesActive() {
        this.providers.forEach(function (provider) {
            this.doCookieAction(provider,true);
            this.addCookieHistory(provider.id, true);
            provider.state = true;
        }.bind(this));
        this.setDCCookieProviderStates();
    }

    /**
     * toggles the cookie information
     *
     * @param id
     * @param toggle
     */
    toggleCookie(id, toggle){
        this.providers.forEach(function (provider) {
            if(provider.id == id){
                if(provider.state !== toggle){
                    this.doCookieAction(provider,toggle);
                    this.addCookieHistory(provider.id, toggle);
                }
                provider.state = toggle;
            }
        }.bind(this));
        this.setDCCookieProviderStates();
    }

    /**
     * activates / deactivates the iframes, scripts and google tag manager injections by class
     *
     * @param provider
     * @param state
     */
    doCookieAction(provider, state){
        if(state === true){
            if (provider.externalMediaContainerClass) {
                let containerElements = document.getElementsByClassName(provider.externalMediaContainerClass);
                for (let j = 0; j < containerElements.length; j++) {
                    let containerElement = containerElements[j];
                    containerElement.innerHTML = "";
                }
            }
            if(provider.iframeclass !== null){
                let cookieFrames = document.getElementsByClassName(provider.iframeclass);
                for (let i = 0; i < cookieFrames.length; i++){
                    if (cookieFrames[i].tagName !== "DIV") continue;

                    // cookieFrames[i].setAttribute('src',cookieFrames[i].dataset.dccookierealdomain);
                    this.replaceElementTag(cookieFrames[i],'iframe');
                }
            }
            if(provider.scriptclass !== null){
                let cookieScripts = document.getElementsByClassName(provider.scriptclass);
                for (let i = 0; i < cookieScripts.length; i++){
                    if (cookieScripts[i].tagName !== "SCRIPT") continue;

                    if(provider.runscript === true && cookieScripts[i].innerHTML !== ''){
                        cookieScripts[i].setAttribute('type','text/javascript');
                        eval(cookieScripts[i].innerHTML);
                    }

                    if (cookieScripts[i].src !== '') {
                        var newEle = document.createElement('script');
                        newEle.type = 'text/javascript';
                        newEle.class = cookieScripts[i].class;
                        newEle.src = cookieScripts[i].src;
                        cookieScripts[i].parentNode.insertBefore(newEle, cookieScripts[i]);
                    }
                }
            }
            if(typeof provider.gtmVariable !== 'undefined'){
                if(!window.dataLayer) {
                    window.dataLayer = [];
                }
                window.dataLayer.push({
                    'event' : [provider.gtmVariable] + 'Event',
                    [provider.gtmVariable] : true
                });
            }
        }else{
            if(provider.iframeclass !== null){
                let cookieFrames = document.getElementsByClassName(provider.iframeclass);
                for (let i = 0; i < cookieFrames.length; i++){
                    if (cookieFrames[i].tagName !== "IFRAME") continue;

                    // cookieFrames[i].setAttribute('src','');
                    this.replaceElementTag(cookieFrames[i],'div');
                }
            }
            if(provider.scriptclass !== null){
                let cookieScripts = document.getElementsByClassName(provider.scriptclass);
                for (let i = 0; i < cookieScripts.length; i++){
                    if (cookieScripts[i].tagName !== "SCRIPT") continue;

                    cookieScripts[i].setAttribute('type','text/plain');
                }
            }
            if(typeof provider.gtmVariable !== 'undefined'){
                if(!window.dataLayer) {
                    window.dataLayer = [];
                }
                window.dataLayer.push({
                    'event' : [provider.gtmVariable] + 'Event',
                    [provider.gtmVariable] : false
                });
            }
        }
    }

    /**
     * adds the cookie by id to the history list
     *
     * @param id
     * @param state
     */
    addCookieHistory(id, state){
        const new_history = {"dcCookieId" : id,"state" : state,"date":Date()};
        this.dcCookieHistory.push(new_history);
        this.setDCCookieHistory();
    }

    /**
     * updates the dcCookieHistory property with the value from the local storage
     *
     * @returns {boolean}
     */
    getCookieHistory(){
        let storage = localStorage;
        var dcCookieHistory;
        var dcCookieHistoryParse;
        if (typeof(storage) !== "undefined" && this.lsTest()) {
            dcCookieHistoryParse = storage.getItem(this.historyStorage);
            if( dcCookieHistoryParse !== "undefined" && dcCookieHistoryParse !== null){
                if(dcCookieHistory = JSON.parse(dcCookieHistoryParse)){
                    this.dcCookieHistory = dcCookieHistory;
                    return true;
                }
            }
            this.dcCookieHistory = [];
            return true;
        }
        this.dcCookieHistory = [];
        return false;
    }

    /**
     * sets the dcCookieHistory in the local storage
     */
    setDCCookieHistory(){
        var storage = localStorage;
        storage.setItem(this.historyStorage,JSON.stringify(this.dcCookieHistory));
    }

    /**
     * gets the states for the providers and sets the status via the function doCookieAction
     */
    getCookieProviderStates(){
        let storage = localStorage;
        var dcCookieProviderStates;
        var dcCookieProviderStatesParse;
        if (typeof(storage) !== "undefined" && this.lsTest()) {
            dcCookieProviderStatesParse = storage.getItem(this.providerStorage);
            if( dcCookieProviderStatesParse !== "undefined" && dcCookieProviderStatesParse !== null){
                if(dcCookieProviderStates = JSON.parse(dcCookieProviderStatesParse)){
                    for(let i=0; i<this.providers.length;i++){
                        for(let j=0; j<dcCookieProviderStates.length;j++){
                            if(this.providers[i].id == dcCookieProviderStates[j].id){
                                this.providers[i].state = dcCookieProviderStates[j].state;
                                if (this.providers[i].state === true) {
                                    this.doCookieAction(this.providers[i], this.providers[i].state);
                                }
                            }
                        }
                    }
                }
            }
        }
    }

    getAllDeclinedExternalMediaProviders() {
        let providers = [];
        for(let i=0; i<this.providers.length;i++){
            let provider = this.providers[i];
            if (!provider.state && provider.category == COOKIE_EXTERNAL_MEDIA) providers.push(provider);
        }
        return providers;
    }

    /**
     * sets the dcCookieProviderStates with the settings into the localStorage
     */
    setDCCookieProviderStates(){
        var storage = localStorage;
        storage.setItem(this.providerStorage,JSON.stringify(this.providers));
        if(!this.getSettingsAlreadySet()){
            this.dcCookieSettings.settings_already_set = true;
            this.setDCCookieSettings();
        }
    }

    /**
     * replaces
     *
     * @param elem
     * @param tag
     */
    replaceElementTag(elem,tag) {
        const replacement = document.createElement(tag)

        for (let i = 0; i < elem.attributes.length; i++) {
            const attr = elem.attributes[i]
            replacement.setAttribute(attr.name, attr.value)
        }

        replacement.innerHTML = elem.innerHTML

        elem.replaceWith(replacement);
    }

    async cookieStatisticAction(action) {
        if (this.enableStatistics) {
            const data = {
                siteCode: this.siteCode,
                action: action,
                state: this.providers.map(function (provider) {
                    return {
                        id: provider.id,
                        state: provider.state
                    }
                }),
                device: this.getDeviceType()
            };

            const postSettings = {
                method: 'POST',
                mode: 'same-origin',
                cache: 'no-cache',
                credentials: 'same-origin',
                headers: {
                    'Content-Type': 'application/json'
                },
                redirect: 'follow',
                referrerPolicy: 'same-origin',
                body: JSON.stringify(data)
            };

            const url = window.location.origin + '/plugins/DCcookie/statistics.php';
            fetch(url, postSettings);
        }
    }

    getDeviceType() {
        const deviceInfo = navigator.userAgent || navigator.vendor || window.opera;
        let mobile = false;
        if (/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i.test(deviceInfo) || /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(deviceInfo.substr(0, 4))){
            mobile = true;
        }
        let mobileTablet = false;
        if (/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino|android|ipad|playbook|silk/i.test(deviceInfo)||/1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(deviceInfo.substr(0, 4))){
            mobileTablet = true;
        }
        if (mobile) return 'mobile';
        if (mobileTablet) return 'tablet';
        return 'desktop';
    }
}


