...
function format_amount($amount, $gramm = false, $itemprop = false)
{
    //Währung abfragen, wenn keine gefunden -> Euro als Standard
    if (array_key_exists('shop_currency', $GLOBALS) && is_array($GLOBALS['shop_currency']) && array_key_exists(
            'code',
            $GLOBALS['shop_currency']
    ...
    if ($gramm) {
        $format = number_format($amount, 2, ',', '.') . " " . $sign;
    } else {
        if ($itemprop) {
            $format = "<span itemprop='price'>" . number_format(
                    $amount,
                    2,
                    ',',
                    '.'
                ) . " <span class='itemprop_price'>" . $snippet . "</span></span> " . $sign;
        } else {
            $format = number_format($amount, 2, ',', '.') . " " . $sign;
        }
    }
...
function shop_add_item_to_basket($visitor_id, $item_id, $quantity = 1, $variant = '')
{
    ...
    if ((@mysqli_num_rows($item_result) > 0) && ($quantity > 0)) {
...
function shop_login_listener()
{
    ...
    switch ($_GET["action"]) {
        ...
        case "shop_login":
                    ...
                    if (password_verify($inputPassword, $dbPasswordHash) || $validOldHash) {
                        ...
                        if ($shop_user['last_visitor_id'] != '') {
                            ...
                            if ($GLOBALS['shop']['shop_typ'] == 1) {
                                ...
                                $basketLoginHandler->handleLogin($visitor['id'], $old_visitor_id, false);
                            }
            ...
            if (isset($_GET['action']) && $_GET['action'] == 'shop_login' && isset($_POST['catalog_selected_item']) && $_POST['catalog_selected_item'] != '') {
            ...
            if (!empty($_POST["redirect_url"])) {
                universal_redirect("//" . $_SERVER["SERVER_NAME"] . $_POST["redirect_url"]);
            }
    ...
    if (@mysqli_num_rows($result) > 0) {
        while ($shipment_address = mysqli_fetch_array($result)) {
            ...
            if ($shipment_address["id"] == $value) {
                echo "<option selected=\"selected\" value=\"" . $shipment_address["id"] . "\">" . $description . "</option>";
            } else {
                echo "<option value=\"" . $shipment_address["id"] . "\">" . $description . "</option>";
            }
        }
    }
    echo "</select></div></div>";
...
function get_text_module($company, $text_module_code, $spacer = array(), $description = false)
{
    if ($text_module_code <> '') {
...
function password_reminder()
{
    ...
    $mail_send = false;
    //if(($_POST["input_customer_no"]<>'') | ($_POST["input_email"]<>'')) {

    /*
      if ($_POST["input_email"] <> '') {
          /*$countquery = "SELECT *
                         FROM shop_user
                         WHERE shop_customer_id = (SELECT id
                                                      FROM shop_customer
                                                      WHERE customer_no = '" . $_POST["input_customer_no"] . "'
                                                          AND company = '".$GLOBALS['shop']['company']."')
                             AND email = '" . $_POST["input_email"] . "'";*/
    /* $countquery = ((!empty($_POST["input_customer_no"])) ? 'SELECT * FROM shop_user WHERE customer_no = \'' . $_POST["input_customer_no"] . '\' AND email=\'' . $_POST['input_email'] . '\'' :
         'SELECT * FROM shop_user WHERE email = \'' . $_POST['input_email'] . '\'');
     ...
     if (@mysqli_num_rows($result) == 1) {
         ...
         $query = "UPDATE shop_user
                   ...
                   WHERE id = '" . $user["id"] . "'";
         if (mysqli_query($GLOBALS['mysql_con'], $query)) {
             $spacer["%customer_no%"] = $_POST["input_customer_no"];
             $spacer["%login%"] = (!empty($user["login"]) ? $user["login"] : $user["email"]);
             $spacer["%password%"] = $password;
             $spacer['%email%'] = $_POST['input_email'];
             if ($GLOBALS["shop_language"]["email_password_text_module"] <> '') {
                 $message = get_text_module(
                     $GLOBALS['shop']['company'],
                     $GLOBALS["shop_language"]["email_password_text_module"],
                     $spacer
                 );
                 ...
                 $query = 'UPDATE shop_user SET password=\'' . $oldpasswordhash . '\' WHERE id=' . $user['id'];
                 mysqli_query($GLOBALS['mysql_con'], $query);
             }
         }
     }
    ...
    switch ($GLOBALS['shop']['login_type']) {
        case 0: //E-Mail & Passwort
            ...
            $countquery = " SELECT * FROM shop_user WHERE company= '" . $GLOBALS['shop']['company'] . "' and shop_code= '" . $GLOBALS['shop']['code'] . "' and  email = '" . $_POST['input_email'] . "'";
            break;
        case 1: //Login & Passwort
            if (!isset($_POST['input_login']) || $_POST['input_login'] == "") {
                $showError = true;
            }
            $countquery = " SELECT * FROM shop_user WHERE company= '" . $GLOBALS['shop']['company'] . "' and shop_code= '" . $GLOBALS['shop']['code'] . "' and  login = '" . $_POST['input_login'] . "'";
            break;
        case 2: //Kunden-Nr., Login & Passwort
            ...
            $countquery = "SELECT * FROM shop_user WHERE   company= '" . $GLOBALS['shop']['company'] . "' and shop_code= '" . $GLOBALS['shop']['code'] . "' and  customer_no = '" . $_POST["input_customer_no"] . "' AND login='" . $_POST['input_login'] . "'";
            break;
        case 3: //Kunden-Nr., E-Mail & Passwort
            ...
            $countquery = "SELECT * FROM shop_user WHERE   company= '" . $GLOBALS['shop']['company'] . "' and shop_code= '" . $GLOBALS['shop']['code'] . "' and  customer_no = '" . $_POST["input_customer_no"] . "' AND email='" . $_POST['input_email'] . "'";
            break;
        case 4: //Kunden-Nr. & Passwort
            if (!isset($_POST['input_customer_no']) || $_POST['input_customer_no'] == "") {
                $showError = true;
            }
            $countquery = "SELECT * FROM shop_user WHERE   company= '" . $GLOBALS['shop']['company'] . "' and shop_code= '" . $GLOBALS['shop']['code'] . "' and  customer_no = '" . $_POST["input_customer_no"] . "'";
            break;
        default:
            $showError = true;
            break;
    ...
    if ($showError !== true) {
        ...
        if (@mysqli_num_rows($result) == 1) {
            ...
            $hashString = $GLOBALS['shop']['company'] . $GLOBALS['shop']['code'] . $user['email'] . rand() . getenv('SHOP_PASSWORD') . '@!#$%^&*' . time();
           ...
           SELECT main_navigation.*  
                FROM main_navigation
                    ...
                    inner join main_page_link on main_page.id = main_page_link.main_page_id  and main_page.main_language_id = :langaugeId
...
)
{
    ...
    <table class="order_sum" style="width:100%">
        ...
        if ($show_vat == 1 && $sales_line_result != null && (!(isset($GLOBALS['us_sales_tax_breakdown']) || isset($GLOBALS['us_sales_tax_estimate'])))) {
            ...
            //Aufschläge
            $markup = $shipping_cost + $payment_cost + $small_quantity_charge_amount;
            ...
            if (isset($GLOBALS['vat_order_visitor_' . $GLOBALS['visitor']['id']][0])) {
                for ($i = 0; $i < count($GLOBALS['vat_order_visitor_' . $GLOBALS['visitor']['id']]); $i++) {
                    if (is_array($basket_vat_groups) && in_array(
                        ...
                        echo("<tr>");
                        echo("<td class=\"order_sum_1\">" . $GLOBALS["tc"]["incl_tax"] . " " . round(
...
)
{
    ...
    switch ($level) {
        case 1:
            $levelcode = array_key_exists('slevel_1', $_GET) ? $_GET["slevel_1"] : '';
            break;
        case 2:
            $levelcode = array_key_exists('slevel_2', $_GET) ? $_GET["slevel_2"] : '';
            break;
        case 3:
            $levelcode = array_key_exists('slevel_3', $_GET) ? $_GET["slevel_3"] : '';
            break;
        case 4:
            $levelcode = array_key_exists('slevel_4', $_GET) ? $_GET["slevel_4"] : '';
            break;
        case 5:
            $levelcode = array_key_exists('slevel_5', $_GET) ? $_GET["slevel_5"] : '';
            break;
        case 6:
            $levelcode = array_key_exists('slevel_6', $_GET) ? $_GET["slevel_6"] : '';
            break;
    }
    $spaces = "";
    $count = 0;
    ...
    if (@mysqli_num_rows($result) > 0) {
        ...
        while ($nav = mysqli_fetch_array($result)) {
            ...
            if (array_key_exists('slevel_' . $layer_no, $_GET) && $_GET["slevel_" . $layer_no] == $nav["code"]) {
                $active = "class=\"active\" ";
            ...
            if ($show_level) {
                ...
                if ($showCategoryIconInNavigationMenu && $full_menu) // for the top menu
                {
                    ...
                    if ($nav['category_icon'] != '' && file_exists("../../" . $categoryIcon)) {
                        ...
                        echo "" . $spaces . "<li class=\"level_$level $active_classname \"><a " . $active . "href=\"/" . customizeUrl() . "/" . $newnavcode . "\">" . $nav["name"] . $categoryIcon . "</a>";
                    } else {
                        echo "" . $spaces . "<li class=\"level_$level $active_classname \"><a " . $active . "href=\"/" . customizeUrl() . "/" . $newnavcode . "\">" . $nav["name"] . "</a>";
                    }
            ...
            if (($nav["code"] == $levelcode) | $full_menu) {
...
{
    if ($name != '') {
        if (count($GLOBALS['category_sort_types']) > 0) {
            echo "<div class=\"" . $sort . "label\"><label for=\"" . $selectname . "\">" . $name . "</label></div>";
            echo "<div class=\"input select_body\"><SELECT name=\"" . $selectname . "\" id=\"" . $selectname . "\" onchange=\"this.form.submit();\">";
            ...
            $i = 0;
            while ($i < count($GLOBALS['category_sort_types'])) {
...
{
    if (!empty($code)) {
        for ($i = 0; $i <= count($GLOBALS['category_sort_types']); $i++) {
...
{
    if (array_key_exists('slevel_6', $_GET) && !empty($_GET["slevel_6"])) {
        $layer_no = "6";
    } elseif (array_key_exists('slevel_5', $_GET) && !empty($_GET["slevel_5"])) {
        $layer_no = "5";
    } elseif (array_key_exists('slevel_4', $_GET) && !empty($_GET["slevel_4"])) {
        $layer_no = "4";
    } elseif (array_key_exists('slevel_3', $_GET) && !empty($_GET["slevel_3"])) {
        $layer_no = "3";
    } elseif (array_key_exists('slevel_2', $_GET) && !empty($_GET["slevel_2"])) {
        $layer_no = "2";
    } elseif (array_key_exists('slevel_1', $_GET) && !empty($_GET["slevel_1"])) {
        $layer_no = "1";
    } else {
        return 0;
    }
    if ((array_key_exists('card', $_GET) && $_GET["card"] <> '') || (array_key_exists(
                'shop_category',
                $_GET
...
)
{
    ...
    if (count($shippingAgents) > 0) {
        ...
        foreach ($shippingAgents as $shippingAgent) {
            ...
            echo "<option " . $selected . " value=\"" . $shippingAgent["line_no"] . "\">" . $shippingAgent["description"] . " (+" . format_amount(
                    $shippingAgent["shipping_cost"],
                    false
                ) . ")</option>";
        }
        echo "</select></div></div>";
...
)
{
    ...
    if (count($shippingAgents) > 0) {
        ...
        foreach ($shippingAgents as $shippingAgent) {
            ...
            $costinfo = "&nbsp;(+" . format_amount($shippingAgent['shipping_cost']) . ")";
            /*} else {
                $costinfo = '';
            }*/
            ...
            echo $img . "<span class=\"span-wrapper\"><span class=\"termDescription\">" . $shippingAgent["description"] . $costinfo . "</span>" . $longText . "</span>";
            echo "</label>";
            echo "</div>";
            echo "</div>";
            $i++;
...
function getAvailableShippingAgents($visitorID, $countrycode, $postCode, UserBasket $basket = null)
{
    ...
    if (null !== $basket) {
        ...
        if ((count($shippingOptions) > 0) && $shippingOptions->getFirst()->getID() > 0) {
    ...
    if (@mysqli_num_rows(mysqli_query($GLOBALS['mysql_con'], $shippingCostQuery)) > 0) {
...
function payment_terms_select($user, $name, $selectname = "input_payment_line_no", $value = NULL, $disabled = FALSE, $autoupdate = FALSE, $countrycode = '', $isSubscriptionOrder = false)
{
    ...
    if ($payment_terms) {
        ...
        while ($payment_term = mysqli_fetch_array($result)) {
            ...
            $costinfo = "&nbsp;(+ " . $numberFormatter->formatCurrency($payment_term['payment_cost'], $configuration->getCurrencyCode() === '' ? 'EUR' : $configuration->getCurrencyCode()) . ")";
            /*} else {
                $costinfo = '';
            }*/
            ...
            echo "<option " . $selected . " value=\"" . $payment_term["line_no"] . "\">" . $payment_term["description"] . $costinfo . "</option>";
            $i++;
        }
        echo "</select></div></div>";
    }
...
function payment_terms_list($user, $name, $selectname = "input_payment_line_no", $value = NULL, $disabled = FALSE, $autoupdate = FALSE, $countrycode = '', $isSubscriptionOrder = false)
{
    ...
    if ($payment_terms) {
        ...
        $autoupdate_text = ($autoupdate) ? " onchange=\"this.form.submit();\"" : "";
        echo "<div class='form-group'><div class='order_devision_headline'>" . $name . "</div></div>";
        echo "<div class=\"paymentTermWrapper order_option_list_inner\"><div class='row'>";
        $result = mysqli_query($GLOBALS['mysql_con'], $payment_terms);
        $i = 0;
        $j = 0;
        if ($_POST["input_payment_line_no"] == '' && $_SESSION['payment_line_no'] == '') {
        ...
        while ($payment_term = mysqli_fetch_array($result)) {
            ...
            if ($payment_term["logo"] != "" && file_exists(
                ...
                $img = "<div class='image'><img src=\"" . $GLOBALS["shop_setup"]["uploaddir_order_icons"] . $payment_term["logo"] . "\"></div>";
            }
            $longText = "";
            if ($payment_term["content"] != "") {
                $longText = "&nbsp;<span class=\"longText\">" . $payment_term["content"] . "</span>";
            }
            echo "<div class='col-xs-12 col-sm-6 col-md-6 col-lg-4'><div class=\"form-check paymentTerm" . $selected . "\">";
            echo "<label  for=\"" . $selectname . "_" . $i . "\">";
            ...
            echo $img . "<span class=\"span-wrapper\"><span class=\"termDescription\">" . $payment_term["description"] . $costinfo . "</span>" . $longText . "</span>";
            echo "</label>";
            echo "</div></div>";
            $i++;
        }
...
function get_available_payment_terms($countrycode, $dc_active = false, $recurrent_payment_active = false, $basket = null)
{
    ...
    $queryCondition = "";
    if (count($allowedPaymentOptionsIds) > 0) {
        $paymentIds = "";
    ...
    if ($_SESSION["dc_id"] == '' && !$dc_active && !$recurrent_payment_active) {
        $payment_terms_query = "SELECT *
							  ...
							  ORDER BY " . $orderBy;
    } elseif ($_SESSION["dc_id"] != '' || $dc_active) {
        $payment_terms_query = "SELECT *
							  FROM shop_payment_option
							  WHERE company = '" . $GLOBALS['shop']['company'] . "'
								AND shop_code = '" . $GLOBALS['shop']['code'] . "'
    ...
    if (@mysqli_num_rows(mysqli_query($GLOBALS['mysql_con'], $payment_terms_query)) > 0) {
...
function check_mandatory_fields_b2c($post_var, $step = "step1")
{
    ...
    if ($GLOBALS["shop_customer"]["customer_no"] == "") {
        ...
        if ($step == "step1" && $_GET["action"] == "step2") {
            ...
            if ($post_var['input_shipping_is_not_invoice'] == "on") {
                if ($post_var["input_surname_shipping"] == "") {
                    $ret_val = false;
                } elseif ($post_var["input_lastname_shipping"] == "") {
                    $ret_val = false;
...
function session_save_data_b2c($page)
{
    if ($page == "step1") {
        ...
        if (isset($_POST["input_name"]) || $_POST["input_surname"]) {
            ...
            $_SESSION["visitor_address"] = $_POST["input_user_street"];
            $_SESSION["visitor_address_2"] = $_POST["input_user_street_2"];
            $_SESSION["visitor_address_no"] = $_POST["input_user_street_no"];
            ...
            $_SESSION["visitor_user_street_shipping"] = $_POST["input_user_street_shipping"];
            $_SESSION["visitor_user_street_no_shipping"] = $_POST["input_user_street_no_shipping"];
            $_SESSION["visitor_user_street_2_shipping"] = $_POST["input_user_street_2_shipping"];
            ...
            $_SESSION["visitor_user_street_shipping_packstation"] = $_POST["input_user_street_shipping_packstation"];
        ...
        $_SESSION["visitor_address"] = $_POST["input_shop_customer_address"];
        $_SESSION["visitor_address_2"] = $_POST["input_shop_customer_address_2"];
...
/*function session_save_data_b2c($page)
{
    if (isset($_POST["input_shipping_is_packstation"]) || $_POST["input_shipping_is_not_invoice"] || $_POST["input_shipping_is_invoice"]) {
        $_SESSION["visitor_radio_shipping_is_packstation"] = $_POST["input_shipping_is_packstation"];
        $_SESSION["visitor_radio_shipping_is_not_invoice"] = $_POST["input_shipping_is_not_invoice"];
    }
    if ($page == "step1") {
...
$_SESSION["bank_no"] = (isset($_POST["input_bank_no"])) ? $_POST["input_bank_no"] : $_SESSION["bank_no"];
$_SESSION["bank_name"] = (isset($_POST["input_bank_name"])) ? $_POST["input_bank_name"] : $_SESSION["bank_name"];
$_SESSION['bonus_code'] = (isset($_POST['input_bonus'])) ? mb_strtolower($_POST['input_bonus'], 'UTF-8') : $_SESSION['bonus_code'];
$_SESSION["your_comment"] = (isset($_POST['input_your_comment'])) ? str_replace("\\r\\n", " ", $_POST["input_your_comment"]) : $_SESSION["your_comment"];
$_SESSION["visitor_birthday"] = (isset($_POST["input_birthday"])) ? $_POST["input_birthday"] : $_SESSION["visitor_birthday"];
} elseif ($page == "start") {
$_SESSION["order_account_type"] = $_POST["input_order_user_account"];
} elseif ($page == "customer_data_save") {
...
function get_meta_description()
{
    ...
    IF ($card > 0) {
        ...
        $GLOBALS['item'] = $item;
        $query_1 = "SELECT meta_description, item_no, parent_item_no FROM shop_view_active_item WHERE id = '" . $card . "' AND company = '" . $company . "' AND shop_code = '" . $shop . "' AND language_code = '" . $language . "'";
        $result_1 = mysqli_query($GLOBALS['mysql_con'], $query_1);
        $row = mysqli_fetch_assoc($result_1);
        $short_desc = $row["meta_description"];
        ...
        IF ($short_desc <> '') {
            ...
            IF ($parent_item_no <> '') {
                $query_2 = "SELECT meta_description FROM shop_view_active_item WHERE item_no = '" . $parent_item_no . "' AND company = '" . $company . "' AND shop_code = '" . $shop . "' AND language_code = '" . $language . "'";
                $result_2 = mysqli_query($GLOBALS['mysql_con'], $query_2);
                $short_desc = mysqli_result($result_2);
                IF ($short_desc <> '') {
                    RETURN $short_desc;
                } ELSE {
    ...
    if ($_SESSION['coupon']['used'] == 1) {
        ...
        if (@mysqli_num_rows($result) == 0) {
            $query2 = "SELECT scl.*, scll.category_coupon, scll.category_line_no, scsl.shipping_coupon_type, scsl.shipping_coupon_value
				  ...
				  AND scl.active = 1
				  AND scl.company = '" . $GLOBALS['shop']['company'] . "'
				  AND scl.shop_code = '" . $GLOBALS['shop']['code'] . "'
				  AND scl.language_code = '" . $GLOBALS['shop_language']['code'] . "'
				  AND (scl.times_used > scl.max_no_of_usage OR scl.max_no_of_usage=0)
				  AND (
...
function user_for_temp_cust_exists()
{
    ...
    return (@mysqli_num_rows(mysqli_query($GLOBALS['mysql_con'], $countquery)) > 0);
}
...
function create_user_for_existing_customer(Customer $customer)
{
    ...
    if (@mysqli_num_rows(mysqli_query($GLOBALS['mysql_con'], $countquery)) == 0) {
...
function user_exists_for_customer($customerNo, $email, &$useID = null)
{
    ...
    $pdo = $IOCContainer->resolve('DynCom\dc\common\classes\PDOQueryWrapper');
    $prepStatement = 'SELECT id FROM shop_user WHERE customer_no = :customerno AND email = :email AND company = :company AND shop_code = :shopcode ORDER BY id LIMIT 1';
    $params = [
...
function register_new_customer($customerNo, $name, $email)
{
    $query = "INSERT INTO shop_customer
                  ...
                  company_name='" . $shipment_address["company_name"] . "',
...
function update_customer($customerNo)
{
    if (!isset($IOCContainer) || !($IOCContainer instanceof IOCInterface)) {
        $IOCContainer = $GLOBALS['IOC'];
    }
...
function show_category_filters($category, $category_sort_order)
{
    ...
    if ($GLOBALS['category']['show_all_items'] && ($category["line_no"] != $GLOBALS["shop_language"]["productfinder_cat_line_no"])) {
        ...
        if (!isset($_GET['card'])) {
            ...
            $result = mysqli_query($GLOBALS['mysql_con'], $query);
            if (@mysqli_num_rows($result) > 0 && $GLOBALS['category']['id'] != '') {
                echo "<div class='filterbox-wrapper'>";
                echo "<div class='filterbox-mobilebutton visible-xs'>" . $GLOBALS['tc']['filterbox-mobilebutton'] . "</div>";
...
function show_all_filters($category_sort_order = '')
{
  ...
  (
    ...
    if (count($always_open) > 0) {
        $order_clause = "ORDER BY CASE WHEN code IN (";
        for ($i = 0; $i < count($always_open); $i++) {
            if ($i > 0) {
                $order_clause .= ",";
            }
...
function show_category_filters_start($category, $category_sort_order = '')
{
    if ($category['show_all_items']) {
        ...
        if (!isset($_GET['card'])) {
            ...
            if (@mysqli_num_rows($result) > 0 && $category['id'] != '') {
...
function show_filter_list($attribute, $items, $for_finder = false, $category_sort_order = '')
{
    ...
    if (count($valuearray) > 0) {
        ...
        <?
        showUnsetFilterButton($attribute);
        echo "</div></div>";
    }
}
...
function show_filter_dropdown($attribute, $items, $for_finder = false, $category_sort_order = '')
{
    ...
    if (count($valuearray) > 0) {
        ...
        showFilterWrapper($attribute);
        echo("<div class='filter dropdown form-group'><div class=\"select_body" . $open_class . " " . $select_class . "\">");
        if ($for_finder) {
            echo("<select class='filterlist' data-attribute-id='" . $attribute["id"] . "'>");
        } else {
            echo("<select  class='filterlist' data-attribute-id='" . $attribute["id"] . "' onchange=\"window.location.href='?" . urlencode(mb_strtolower(str_replace(' ', '~|~', $attribute['code']), 'UTF-8')) . "='+this.options[this.selectedIndex].value+'" . $category_sort_order . "'\">");
        }
        if (isset($_SESSION['filters'][$attribute['code']])) {
            echo("<option value='0'  disabled selected>Alle</option>");
        } else {
            echo("<option value='0'  disabled selected>" . $translation . "</option>");
        }
        foreach ($valuearray AS $key => $value) {
            ...
            if (isset($_SESSION['filters'][$attribute['code']])) {
                if ($_SESSION['filters'][$attribute['code']] == mb_strtolower($key, 'UTF-8')) {
                    echo("<option value='" . urlencode(
                            mb_strtolower($key, 'UTF-8')
                        ) . "' " . $selected . ">" . $translation . ": " . $value . "</option>");
                }
            } else {
                echo("<option value='" . urlencode(
                        mb_strtolower($key, 'UTF-8')
...
function show_filter_slider($attribute, $items, $for_finder = false, $category_sort_order = '')
{
    if (isset($_SESSION['filters'][$attribute['code']])) {
        $filter_set = true;
        $open_class = " open";
...
function show_filter_checkbox($attribute, $items, $for_finder = false, $category_sort_order = '')
{
    ...
    if (count($valuearray) > 0) {
        ...
        if (isset($_SESSION['filters'][$attribute['code']])) {
            ...
            if ($attribute['data_type'] != 3) {
                echo "<div class='filter_headline'>" . $translation . "</div>";
            ...
            <div class="filter_toggle">
                <div class="filter_body icons_body <?= $select_class ?>">
                    <? foreach ($valuearray AS $key => $value) {
                        ...
                        if (file_exists($GLOBALS["shop_setup"]["uploaddir_filter_icon"] . $option['icon'])) {
                            echo("<img src = '" . $GLOBALS['projectRoot'] . $GLOBALS["shop_setup"]["uploaddir_filter_icon"] . $option['icon'] . "' alt='" . $option['description'] . "'>");
                        } else {
...
{
    echo "<div class='col-xs-6 col-sm-4 col-md-4 col-lg-3 filter-" . $attribute['code'] . "-wrapper filter-wrapper'><div class='filter-wrapper-inner'>";
}

/**
 * @param $attributeRow
 */
function showUnsetFilterButton(array $attributeRow): void
{
    ...
    if ((isset($_SESSION['filters'][$rawCode]) || isset(
        ...
        if ($attributeRow["data_type"] != 3 && $_GET[$normalizedCode] != '0'
            ...
            <div class="filter_unset">
                <a href="?<?= urldecode($normalizedCode) ?>=0"
                   title="<?= $GLOBALS['tc']['unset_filter'] ?>"><i class="fa fa-times"
            ...
            ?>
            <div class="filter_unset">
                <a href="?<?= urldecode($normalizedCode) ?>=default"
                   title="<?= $GLOBALS['tc']['unset_filter'] ?>"><i class="fa fa-times"
                                                                    aria-hidden="true"></i></a>
            </div>
            <?
...
function getUnsetFilterButton($attr_code, $attr_display_type, $attr_data_type)
{
    ...
    if ((isset($_SESSION['filters'][$attr_code]) || isset(
        ...
        if ($attr_data_type != 3 && $_GET[$normalizedCode] != '0'
            ...
            $buttonStr .= '
            <div class="filter_unset">
                <a href="?' . urldecode($normalizedCode) . '=0"title="' . $GLOBALS['tc']['unset_filter'] . '"><i class="fa fa-times" aria-hidden="true"></i></a>
            </div>
            ';
            ...
            $buttonStr .= '
            <div class="filter_unset">
                <a href="?' . urldecode($normalizedCode) . '=default" title="' . $GLOBALS['tc']['unset_filter'] . '"><i class="fa fa-times" aria-hidden="true"></i></a>
            </div>
            ';
        }
    }
...
function get_option_values($attribute, $items)
{
    ...
    if (!$options) {
        ...
        $optionvaluequery = "SELECT shop_attribute_option.id,shop_attribute_option.id,shop_attribute_option.company,shop_attribute_option.code,shop_attribute_option.attribute_code,shop_attribute_option.filter,shop_attribute_option.icon,shop_attribute_option.sorting,
                             CASE WHEN
                                (SELECT count(id) FROM shop_attribute_translation where company = '" . $GLOBALS['shop']['company'] . "' AND type = shop_attribute_link.type AND attribute_code = shop_attribute_link.attribute_code AND attribute_link_no = shop_attribute_link.no AND language_code = '" . $GLOBALS['shop_language']['code'] . "') > 0
                            THEN
                                (SELECT description FROM shop_attribute_translation where company = '" . $GLOBALS['shop']['company'] . "' AND type = shop_attribute_link.type AND attribute_code = shop_attribute_link.attribute_code AND attribute_link_no = shop_attribute_link.no AND language_code = '" . $GLOBALS['shop_language']['code'] . "')
                            ELSE
        ...
        while ($option = mysqli_fetch_assoc($optionsresult)) {
            ...
            //if($option['filter'] == '')
            if (1 == 1) {
                if (in_array($option['code'], $optionvaluearray, true)) {
                    /*$linkquery = "SELECT *
								  FROM shop_attribute_link
								  WHERE attribute_code = '".$attribute['code']."'
        ...
        foreach ($_SESSION['filters'] as $key => $value) {
            ...
            if ($attribute['navision_value'] != 0) {
                ...
                if ($attribute['display_type'] == 3) {
                    ...
                    $itemquery .= " AND v_i." . $field . ">='" . $from . "'";
                    $itemquery .= " AND v_i." . $field . "<='" . $to . "'";
                    ...
                    if (@mysqli_num_rows($optionresult) == 0) {
                        ...
                        if (1 == 1) {
                            ...
                            $itemquery .= " AND v_i." . $field . ">='" . $min . "'";
                            $itemquery .= " AND v_i." . $field . "<='" . $max . "'";
                        }
                    }
                }
            } else {
                ...
                if ($attribute['display_type'] == 3) {
                    ...
                    if (@mysqli_num_rows($optionresult) == 0) {
                        ...
                        if (1 == 1) {
                            ...
                            $linkquery .= " (sial." . $field . ">='" . $min . "'";
                            $linkquery .= " AND sial." . $field . "<='" . $max . "')";
                        }
                    }
                }
            }
...
function get_permissions_group_customer()
{
    ...
    $numrows = @mysqli_num_rows($result_permission_customer);
    $permission_customer = "";
    for ($i = 1; $i <= $numrows; $i++) {
        ...
        if ($i == $numrows) {
            $permission_customer .= " OR (shop_permissions_group_link.permission_group_code IS NULL 
									  ...
									  OR shop_permissions_group_link.customer_no = '" . $GLOBALS['shop_customer']['customer_no'] . "')";
        }
    }
    if ($permission_customer == "") {
        $permission_customer .= "AND (shop_permissions_group_link.customer_no = '" . $GLOBALS['shop_customer']['customer_no'] . "'
...
function generate_pdf($dc_id)
{
    ...
    $message = get_text_module($GLOBALS['shop']['company'], $GLOBALS["shop_language"]["text_email_digital_coupon"]);
    $subject = get_text_module(
        $GLOBALS['shop']['company'],
        $GLOBALS["shop_language"]["text_email_digital_coupon"],
        '',
        true
    );
...
function insert_dc_code($dc_id, $coupon_code)
{
    ...
    $stmt->bindValue(':shopCode', $GLOBALS['shop']["code"], PDO::PARAM_STR);
    $stmt->bindValue(':shopLanguageCode', $GLOBALS['shop_language']["code"], PDO::PARAM_STR);
...
)
{
    ...
    if (null === $basket) {
        ...
        $GLOBALS['vat_order_visitor_' . $visitor_id] = array_values($basket->getItemVATData());
    ...
    if ($markup > 0) {
        if (isset($GLOBALS['vat_order_visitor_' . $visitor_id][2]['vat_percent'])) {
            $GLOBALS['vat_order_visitor_' . $visitor_id][2]['total_amount'] += $markup;
            $GLOBALS['vat_order_visitor_' . $visitor_id][2]['vat_amount'] = 0;
            $GLOBALS['vat_order_visitor_' . $visitor_id][2]['markup_amount'] = $markup;
            $GLOBALS['vat_order_visitor_' . $visitor_id][2]['markup_amount_wo_vat'] = (($markup / (100 + $GLOBALS['vat_order_visitor_' . $visitor_id][2]['vat_percent'])) * 100);
        } elseif (isset($GLOBALS['vat_order_visitor_' . $visitor_id][1]['vat_percent'])) {
            $GLOBALS['vat_order_visitor_' . $visitor_id][1]['total_amount'] += $markup;
            $GLOBALS['vat_order_visitor_' . $visitor_id][1]['vat_amount'] = ($GLOBALS['vat_order_visitor_' . $visitor_id][1]['total_amount'] / (100 + (float)$GLOBALS['vat_order_visitor_' . $visitor_id][1]['vat_percent'])) * $GLOBALS['vat_order_visitor_' . $visitor_id][1]['vat_percent'];
            $GLOBALS['vat_order_visitor_' . $visitor_id][1]['markup_amount'] = $markup;
            $GLOBALS['vat_order_visitor_' . $visitor_id][1]['markup_amount_wo_vat'] = (($markup / (100 + $GLOBALS['vat_order_visitor_' . $visitor_id][1]['vat_percent'])) * 100);
        } else {
            $GLOBALS['vat_order_visitor_' . $visitor_id][0]['total_amount'] += $markup;
            $GLOBALS['vat_order_visitor_' . $visitor_id][0]['vat_amount'] = ($GLOBALS['vat_order_visitor_' . $visitor_id][0]['total_amount'] / (100 + (float)$GLOBALS['vat_order_visitor_' . $visitor_id][0]['vat_percent'])) * $GLOBALS['vat_order_visitor_' . $visitor_id][0]['vat_percent'];
            $GLOBALS['vat_order_visitor_' . $visitor_id][0]['markup_amount'] = $markup;
            $GLOBALS['vat_order_visitor_' . $visitor_id][0]['markup_amount_wo_vat'] = (($markup / (100 + $GLOBALS['vat_order_visitor_' . $visitor_id][0]['vat_percent'])) * 100);
        }
    }
    ...
    while ($row = @mysqli_fetch_assoc($result)) {
        foreach ($importance_mapping AS $key => $importance) {
            if (!empty($row[$key])) {
                ...
                $prepared_str = strtolower(preg_replace('/\s\d+\s/', ' ', $prepared_str_2));
...
//Sonderkategorien
    if ($_GET['shop_category'] == 'search') {
        ...
        if (is_array($GLOBALS['category']) || is_int($_GET['card'])) {
            if (!(int)$_GET['card'] > 0) {
    ...
    try {
        ...
        foreach ($GLOBALS['active_campaigns'] as $campaign) {
            if ($campaign instanceof \DynCom\dc\dcShop\campaigns\GenericCampaign) {
                $title = $campaign->getDescription();
                $actionElements = $campaign->getActionElements();
                $hasItemCampaignAction = (count($actionElements) > 0) && $actionElements->itemMeetsCriteria(
                        $itemWithCategories
                    );
                if ($hasItemCampaignAction && !empty($campaign->getTextActionItems())) {
                    $notifications['actions'][] = ['text' => get_text_module($GLOBALS['shop']['company'],$campaign->getTextActionItems()), 'icon' => $campaign->getIconPathActionItems(),'banner' => $campaign->getBannerPathActionItems()];
                }
                $conditionElements = $campaign->getConditionElements();

                $itemMeetsCampaignConditions = (count(
                            $conditionElements
                        ) === 0) || $conditionElements->itemMeetsCriteria($itemWithCategories);
                if ($itemMeetsCampaignConditions && !empty($campaign->getTextConditionItems())) {
                    $notifications['conditions'][] = ['text' => get_text_module($GLOBALS['shop']['company'],$campaign->getTextConditionItems()), 'icon' => $campaign->getIconPathConditionItems(),'banner' => $campaign->getBannerPathConditionItems()];
                }
                $basketText = $campaign->getTextBasket();
                if ($_GET['shop_category'] === 'basket' && $basketText) {
                    $notifications['basket'][] = ['text' => $basketText, 'icon' => $campaign->getBannerBasket()];
                }
...
function logout_customer_invalid_language(&$visitor, $shopLanguage, $dbConn)
{
    ...
    if (is_array($visitor) && $dbConn instanceof mysqli && isset($visitor['id']) && !empty($shopLanguage)) {
        ...
        if ($visitor['frontend_login']) {
            ...
            if (is_array($arr) && count($arr) > 0 && isset($arr[0]['language_code'])) {
                $language_code = $arr[0]['language_code'];
...
function get_shop_breadcrumb()
{
    ...
    if (isset($_GET['shop_category']) && $_GET['shop_category'] != "dc_order" && $_GET['shop_category'] != "queue") {
        ...
        if ($_GET["shop_category"] == 'account') {
            $breadcrumb_shopcategory = (empty($_GET['action']) ? $GLOBALS['tc']['shop_account'] : $GLOBALS['tc'][$_GET['action']]);
        }
        ...
        if ($category["id"] <> '') {
            ...
            if ($_GET["slevel_2"]) {
                ...
                $query = "SELECT * FROM  `shop_item` WHERE  `id` ='" . $_GET["card"] . "'";
                $result = @mysqli_query($GLOBALS['mysql_con'], $query);
                if (@mysqli_num_rows($result) == 1) {
                    $item = @mysqli_fetch_assoc($result);
        ...
        <? if (isset($_GET['card'])) { ?>
            <div class="main_content_left col-xs-12 col-sm-4 col-md-3 col-lg-3">
                <? if (isset($_GET['card'])) {
                    ...
                    $breadcrumb .= '&nbsp;<i class="fa fa-angle-right" aria-hidden="true"></i>&nbsp;<span>' . htmlentities(
                    ...
                    echo "<div class='itemcard_back_button'>";
                    if ($_GET['search'] == 1) {

                        ?>
                        <a class="button_back"
                           href="/<? echo customizeUrl(); ?>/search/<?= $pageParameter ?>&term=<?= $_SESSION['search'] ?>#item_<?= $itemNo ?>"><?= $GLOBALS["tc"]["back_to_search"] ?></a>
                    <? } else {
                        if ($pageParameter != '') {
                            $pageParameter = '?page=' . $_GET['page'];
                        }
                        ...
                        if ($_GET['card']) {
                            $backurl = category_get_path($category) . $itemObj->getDescription() . "-p" . $id . "/";
                        }
                        ...
                        if ($GLOBALS['category'] == NULL && $_SESSION['search'] == NULL && !isset($_GET['card'])) {
                            echo "<a class='button_back' href='/" . customizeUrl() . "/'>" . $GLOBALS['tc']['back_startpage'] . "</a>";
                        } else {
                            echo $back_url;
                        }
                    }
...
function create_password_reminder_html()
{
    ...
    if (count($result) == 1) {
        ...
        $show_labels_in_fields ?
            "
					<div class=\"form-group\">
                        <input type=\"text\" class=\"form-control\" id=\"input_email_reminder\" name=\"input_email\" placeholder=\"" . $GLOBALS['tc']['email'] . "\">
                    </div>
					"
            :
            "
                    <div class=\"form-group\">
                        ...
                        <input type=\"text\" class=\"form-control\" name=\"input_email\" id=\"input_email_reminder\">
                    </div>
				"
        );
        ...
        $show_labels_in_fields ?
            "
                        <div class=\"form-group\">
                            <input type=\"text\" class=\"form-control\" name=\"input_login\" id=\"input_login__reminder\" placeholder=\"" . $GLOBALS['tc']['login_b2b'] . "\">
                        </div>
                        "
            :
            "
                        <div class=\"form-group\">
                            ...
                            <input type=\"text\" class=\"form-control\" name=\"input_login\" id=\"input_login__reminder\">
                        </div>
                    "
        );
        ...
        $show_labels_in_fields ?
            "
                        <div class=\"form-group\">
                            <input type=\"text\" class=\"form-control\" name=\"input_customer_no\" id=\"input_customer_no__reminder\" placeholder=\"" . $GLOBALS['tc']['customer_no'] . "\">
                        </div>
                        "
            :
            "
                        <div class=\"form-group\">
                            ...
                            <input type=\"text\" class=\"form-control\" name=\"input_customer_no\" id=\"input_customer_no__reminder\">
                        </div>
                    "
        );
    }
...
function create_facet_html($attr_code, $attr_desc, $attr_display_type, $attr_data_type, array $options, $sort_code)
{
    ...
    foreach ($options as $optArr) {
        ...
        $selectedStr = (array_key_exists($normalizedAttrCode, $_GET) || array_key_exists($normalizedAttrCodeFrom, $_GET) || array_key_exists($normalizedAttrCodeTo, $_GET)) && $_GET[$normalizedAttrCode] === $opt_code ? ' selected=\'selected\'' : '';
        if ($selectedStr) {
            $optionSelected = true;
        }
...
//Filter in Session setzen, wenn get-parameter vorhanden
    ...
    foreach ($attributesInFilteredDocs as $att) {
        ...
        if ($att['display_type'] != 3) {
            if (array_key_exists($normalizedAttrCode, $_GET) && $_GET[$normalizedAttrCode] !== '') {
...
function create_amazon_payment_button()
{
    ...
    if (count($amazonPay) > 0) {
        if (array_key_exists('shop_currency', $GLOBALS) && is_array($GLOBALS['shop_currency'])
            && array_key_exists('code', $GLOBALS['shop_currency']) && $GLOBALS['shop_currency']['code'] != ''
        ) {
        ...
        $IOCContainer = $GLOBALS['IOC'];
        $currUserBasket = $IOCContainer->create('$CurrUserBasket');
        $_SESSION['amazon_payment_amount'] = $currUserBasket->getBasketTotal();
        $_SESSION['site'] = $GLOBALS['site']['code'];
        $_SESSION['language'] = $GLOBALS['language']['code'];
        ?>
            ...
            window.onAmazonPaymentsReady = function () {
                ...
                OffAmazonPayments.Button('AmazonPayButton', '<?= $GLOBALS["amazon_pay"]["seller_id"] ?>', {
                    ...
                    authorization: function () {
                        ...
                        authRequest = amazon.Login.authorize(loginOptions, 'https://2017.dc-test.de/module/dcshop/b2c/amazon_checkout.inc.php?UserData=<?= session_id() ?>&shopdata=<?  echo $GLOBALS["shop"]["company"] . "|" . $GLOBALS["shop"]["code"]; ?>');
                    },
...
function get_amazon_pay_data($company, $shopCode, $shopLanguageCode)
{
    ...
    $pdo = new \DynCom\dc\common\classes\PDOQueryWrapper($pdoHost, $pdoPort, $pdoSchema, $pdoUser, $pdoPass);
    // Get PayPal Express types with checkout number 23
    $prepStatement = "SELECT 
                        *
                         FROM 
                         shop_payment_option
