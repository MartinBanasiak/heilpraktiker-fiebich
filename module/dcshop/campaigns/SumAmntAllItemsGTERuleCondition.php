 ...
 * Time: 09:47
 */
class SumAmntAllItemsGTERuleCondition implements RuleCondition
{
    ...
    public function __construct(GenericCampaign $campaign,WebshopItemBuilder $itemBuilder)
    {
        ...
        $this->gteThresholdValue = (float)$campaign->getCondSumAmntAllItemsGTE();
    ...
    public function evaluatesTrue(RuleContext $context)
    {
        ...
        if ($basket instanceof UserBasket) {
            ...
            if($sumAmnts >= $this->gteThresholdValue) {
                ...
                $lastItem = end($filteredBasketItems);
                if(!$lastItem instanceof BasketEntity) {
                ...
                $this->extractedVariableArray[] = new GenericRuleContextVariable($this->ruleKey . '.CurrConditionItem',$lastItem);
