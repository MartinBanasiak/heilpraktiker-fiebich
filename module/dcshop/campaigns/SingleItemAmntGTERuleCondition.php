 ...
 * Time: 10:12
 */
class SingleItemAmntGTERuleCondition implements RuleCondition
{
        ...
        if ($basket instanceof UserBasket) {
            ...
            foreach ($filteredBasketItems as $basketEntity) {
                if ($basketEntity instanceof BasketEntity) {
                    $identAmnt = $basketEntity->getLineAmount();
                    ...
                    if (!array_key_exists($identKey, $identKeyData)) {
                        $identKeyData[$identKey] = ['amnt' => $identAmnt, 'entities' => $entities];
                    } else {
                        $identAmnt += (float)$identKeyData[$identKey]['amnt'];
                        $entities += $identKeyData['entities'];
                        $identKeyData[$identKey]['amnt'] = $identAmnt;
                        $identKeyData[$identKey]['entities'] = $entities;
                    }
                }
            }
            foreach ($identKeyData as $identKey => $data) {
                ...
                if ($amnt >= $this->gteThresholdValue) {
                    $atLeastOneTrue = true;
                    //$key = $basket->getKey($basketEntity);
                    $metByFactor = 1;
                    if ($this->gteThresholdValue > 0) {
                        $metByFactor = floor($basketEntity->getLineAmount() / $this->gteThresholdValue);
                    }
                    $matchingBasketItems += $entities;
                    $conditionItemMatchingData[$identKey] = ['threshold_met_by_factor' => $metByFactor];
