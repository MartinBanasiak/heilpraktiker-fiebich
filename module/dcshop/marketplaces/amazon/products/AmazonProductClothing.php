...
class AmazonProductClothing {
    ...
    protected $variationAttributeOrder = array("Size", "Color");
    ...
    public function insert_parentage_variantion_tag($bulkXml, $item, $item_has_category, $VariationDataTag, $is_parent = false, $attributeresult = null, $allVariationAttributes) {
        ...
        $variationTheme = $this->get_amazon_variation_theme($item_has_category, $allVariationAttributes, "Clothing");
    ...
    public function add_attributes($bulkXml, $item, $VariationDataTag, $ClassificationDataTag, $attributeresult) {
        $itemAttributes = array();
        while ($attributeRow = mysqli_fetch_assoc($attributeresult)) {
        ...
        foreach($itemAttributes as $attribute_code => $value) {
            ...
            switch($attribute_code) {
                ...
                case "Color":
                    ...
                    break;
                default:
                    $ClassificationDataTag->appendChild(
                        $bulkXml->createElement($attribute_code, $value)
                    );
                    break;
            }
    ...
    public function create($bulkXml, $item, $item_has_category, $parent, $xsdRow, $variations, $product, $allVariationAttributes) {
        ...
        // bei Argo faengt das ganze mit Clothing an
        $productDataHeader = $productData->appendChild(
            $bulkXml->createElement("Clothing")
        );
        ...
        $ClassificationDataTag = $productDataHeader->appendChild(
            $bulkXml->createElement("ClassificationData")
        );

            // <ProductData><Clothing><ClassificationData><ClothingType>
            $ClassificationDataTag->appendChild(
                $bulkXml->createElement("ClothingType", $product_type)
            );
        ...
        if($variations === true) {
            ...
            if($variationTag === false) {
                ...
                do { // einmal durchlaufen
                    ...
                    $attributeresult = get_item_attributes($item, $all_shop_attributes, AmazonMarketplace::ATTRIBUTE_TYPE);
                    $this->add_attributes($bulkXml, $item, $VariationDataTag, $ClassificationDataTag, $attributeresult);
                } while(false);
            }
        }
        ...
        // VariationData wird nicht benoetigt, wenn keine Attribute oder parent-child infos nicht hinzugefuegt wurden
        if($this->addedVariationTagAttributes === false) {
            $productDataHeader->removeChild($VariationDataTag);
        }
    }
