...
if (($_GET["shop_category"] == 'search') && ($_REQUEST["input_search"] <> '' || $_SESSION['search'] != "") && ($_REQUEST["input_search"] != $GLOBALS["tc"]["search_term"]) && !isset($_GET["card"])) {
    ...
    if ($_POST['input_search'] != "") {
        $_SESSION['search'] = $_POST['input_search'];
    }
    if ($_POST['input_search'] == "" && isset($_SESSION['search'])) {
        $_POST['input_search'] = $_SESSION['search'];
    }
    ...
    $query  = "SELECT *
    		  ...
    		  WHERE item_reference_no LIKE '%" . $_POST['input_search'] . "%'
    ...
    $query  = "SELECT DISTINCT item_no
    		  ...
    		  WHERE content LIKE '%" . $_POST['input_search'] . "%'
    		  	AND company ='" . $GLOBALS['shop']['company'] . "'
    ...
    if ($search_results > 0) {
        ...
        echo "<br /><div class=\"errorbox\">" . $GLOBALS["tc"]["no_item_found"] . "</div>";
    }
...
// Vertreterkonto anzeigen
if (($_GET["shop_category"] == 'salesperson_account') && !isset($_GET["card"])) {
    require __DIR__ . DIRECTORY_SEPARATOR . 'shop_salesperson_account.inc.php';
    $special_content = TRUE;
}
...
if (($_GET['shop_category'] == 'rma') && !isset($_GET['card'])) {
    ...
    include rtrim(dirname(dirname(dirname(__DIR__))),'/\\') . DIRECTORY_SEPARATOR . 'module/dcshop/rma/init.php';
...
if (!$special_content) {
    ...
    if ($category["show_all_items"] == 1 || $category["show_all_items"] == 2 || $category["show_all_items"] == 3) {
        ...
        if ($category['user_sorting']) {
            ...
            if ($_REQUEST["sort_by"] == "") {
                $category_sort_order = get_shop_category_sort_type(3);
            } else {
...
//						echo("<div class='seperator_line'></div>");
                ...
                if ($category["show_random_items"] != 0) {
                    if ($category["show_campain_items"] == 0) {
                        echo "<div class=\"seperator_line_2\"><div style=\"height:10px;margin-top:13px;\">" . $GLOBALS["tc"]["highlights"] . "</div></div>";
                    }
