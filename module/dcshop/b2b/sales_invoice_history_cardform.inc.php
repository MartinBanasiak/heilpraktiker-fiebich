...
<form id="<?= $formname ?>" name="<?= $formname ?>" method="post" class="form-label-left"><input
    ...
    <div
        class="button_row"><?= button("edit button", $GLOBALS["tc"]["list"], $formname, "?action=sales_invoice_history"); ?>
    ...
    if ($_GET['require'] == 1) {
        ...
        <div class="infobox infobox-mailsend">
            ...
            echo "<br />";
            echo button("big button", $GLOBALS["tc"]["require_doc"], $formname, "?action=sales_invoice_history&action_id=card&input_id=" . (int)$_GET['input_id'] . "&update_insert=1");
            ?><br />
            <br />
        </div>
    <?
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-8 col-lg-6">
            <h3><?= $GLOBALS["tc"]["user_invoice_shipment"] ?>: <?= $sales_invoice_header["no"] ?></h3>
            <? input_shop($GLOBALS["tc"]["sales_invoice_date"], "input_shop_customer_no", "code", datefromsql($sales_invoice_header["posting_date"]), 30, TRUE) ?>
            <? input_shop($GLOBALS["tc"]["customer_no"], "input_shop_customer_no", "code", $sales_invoice_header["sell_to_customer_no"], 30, TRUE) ?>
            <div class="spacer_6">&nbsp;</div>
            <? input_shop($GLOBALS["tc"]["name"], "input_name", "text", $sales_invoice_header["sell_to_name_2"] . " " . $sales_invoice_header["sell_to_name"], 30, TRUE) ?>
            <? input_shop($GLOBALS["tc"]["email"], "input_email", "text", $GLOBALS["shop_customer"]["email"], 50, TRUE) ?>
            <div class="spacer_6">&nbsp;</div>
            <? input_shop($GLOBALS["tc"]["your_reference"], "input_your_reference", "text", $sales_invoice_header["your_reference"], 30, TRUE) ?>
        </div>
            ...
            <tbody>
                <tr>
                    ...
                    <td class="order_sum_2 text-right">
                        <?=format_amount($sales_invoice_header["amount"], FALSE)?>
                    </td>
                </tr>
                <? if($sales_invoice_header["amount"] > 0 && $sales_invoice_header["amount_including_vat"] > 0){ ?>
                    <tr>
                        <td class="order_sum_1">

                            <?= $GLOBALS["tc"]["incl_tax"] . " " . calculate_vat($sales_invoice_header["amount"], $sales_invoice_header["amount_including_vat"]) . " %"  ?>
                        </td>
                        <td class="order_sum_2 text-right">
                            <?=format_amount($sales_invoice_header["amount_including_vat"] - $sales_invoice_header["amount"], FALSE)?>
                        </td>
                    </tr>
                <? } ?>
                <tr>
                    ...
                    <td class="order_sum_2 text-right">
                        <div class="order_price_total">
                            <?=format_amount($sales_invoice_header["amount_including_vat"], FALSE)?>
