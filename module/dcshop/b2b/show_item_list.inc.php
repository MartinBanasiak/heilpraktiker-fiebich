        ...
        switch ($type) {
            case 1:
                ...
                if ($actualURLPartsQuery !== '') {
                    $action .= $actualURLPartsQuery . '&';
                }
                //show_item_list_1_header($formname);
                echo "<form name='".$formname."' id='".$formname."' method='POST' action=\"" . $action . "action=shop_add_all_items_to_basket\"><div class=\"itemcard_list1  itemcard_list\"><div class='row'>";
                      $script = '
...
function show_item_list_1( $item, $image, $formname, &$input_counter ) {
   ...
   if (2 == $GLOBALS['shop']['variant_typ']) {
    ...
    $lowestVariantPrice = $item['customer_price'];

    $variantsArray = item_obj_array_to_array($variantsArray);
    foreach ($variantsArray as $variant) {
        $locCustPrice = (float)$variant['customer_price'];
        if ($locCustPrice < $lowestVariantPrice) {
            $lowestVariantPrice = $locCustPrice;
        }
    }
   ...
   if ($image["filename"] == ''  || !file_exists("../.." . $imagelink ) || $image["filename"] == 'noimage.jpg'){
    ...
    if ($GLOBALS['shop']['variant_typ'] != '2') {
        ...
        if ($no_of_variants > 0) {
            ...
            show_item_list_1_has_variant($item, $imagelink, $itemlink, $toggle_text, $formname, $alt_text,$lowestVariantPrice,$no_of_variants);
            $result              = @mysqli_query($GLOBALS['mysql_con'], $query);
...
function show_item_list_1_is_variant( $item, $imagelink, $itemlink, $formname, $last_child = false, $alt_text = '', &$input_counter ) {
    ...
    if ($imagelink == '') {
        ...
        $img_tag = "<img src=\"$imagelink\"  title=\"$alt_text\" alt=\"$alt_text\" />";
    }
    ...
    if (array_key_exists('item_obj',$item) && is_object($item['item_obj'])) {
     ...
     if ($GLOBALS['shop']['variant_typ'] == '0' ||$GLOBALS['shop']['variant_typ'] == '1' ) {
            $itemNoVarCodeText = $GLOBALS['tc']['item_no'];
            $itemNoVarCodeVal = $item['item_no'];
     } elseif ($GLOBALS['shop']['variant_typ'] == '2') {
            $itemNoVarCodeText = $GLOBALS['tc']['variant_code'];
            $itemNoVarCodeVal = $item['variant_code'];
     }

     $locale = $GLOBALS['language']['locale_code'] ?? 'de-DE';
     $numberFormatter = NumberFormatter::create($locale,NumberFormatter::CURRENCY);
                ...
                <a href="<?= $itemlink ?>">
                    ...
                    <div class="itemlist_details_container">
                        <div class="itemlist_content description itemlist_details_top">
                             <div class="itemlist_destcription"><?= $item["description"] ?></div>
                        ...
                        <div class="itemlist_details_right">
                            <div class="itemlist_price_line itemlist_list_price">
                                <span class="itemlist_price_desc"><?= $GLOBALS['tc']['ret_price']?></span>
                                <span class="itemlist_price_val"><?= $numberFormatter->formatCurrency($item['base_price'],$item['currency_code'] ?? 'EUR') ?></span>
                            </div>
                            <?php
                                $graduatedPriceArr = $item['graduated_prices'] ?? [];
                                foreach ($graduatedPriceArr as $gradPrice) {
                                    if ($gradPrice['minimum_quantity'] > 1) {
                                    ?>
                                <div class="itemlist_price_line">
                                    <span class="itemlist_price_desc"><?= $GLOBALS['tc']['your_price_from'] . ' ' . $gradPrice['minimum_quantity'] . ' ' . $item['base_unit_of_measure'] ?></span>
                                    <span class="itemlist_price_val"><?= $numberFormatter->formatCurrency($gradPrice['your_price_per_unit'],$gradPrice['currency_code'] ?? 'EUR') ?></span>
                                </div>
                                    <?php
                                    }
                                }
...
function show_item_list_1_has_variant( $item, $imagelink, $itemlink, $toggle_text, $formname, $alt_text = '', $lowestCustPrice, $noOfVariants ) {
    $locale = $GLOBALS['language']['locale_code'];

    $formatter = NumberFormatter::create($locale,NumberFormatter::CURRENCY);
    $currCode = $GLOBALS['shop_currency']['code'] ?? 'EUR';
    $formattedLowestCustPrice = $formatter->formatCurrency($lowestCustPrice,$currCode);
    ...
    <div id="item_<?= $item["item_no"] ?>" class="has_variant itemlist">
        <div class="itemlist_container">
            ...
            <div class="itemlist_right">
                <div class="no_of_variants"><?= (int)$noOfVariants . ' ' . $GLOBALS['tc']['variants']?></div>
                <div class="button button_action open_variants"><?=$GLOBALS['tc']['show_variants']?></div>
                <div class="from_price"><div class="itemcard_order_button_cust_price_wrapper base_price"><?= $GLOBALS['tc']['from'] . ' ' . $formattedLowestCustPrice ?></div></div>

                <!--<div class="itemlist_content price">
                    <div class="itemlist_content_label"><?=$GLOBALS['tc']['your_price']?></div>
                ...
                <div class="itemlist_content favorites">
                    <div class="itemlist_content_label"><?=$GLOBALS['tc']['favorites']?></div>
                    <? //get_favorite_sign($item) ?>
                </div>-->
            </div>
        </div>
    </div>
    <?/*<tr id="item_<?= $item["item_no"] ?>" class="hasvar hasvar_<?= $item["item_no"] ?> vars_closed"
        ...
        <td width="45" class="list1_image" align="center" onclick="window.location.href='<?= $itemlink ?>'">
            <div><img src="<?= $imagelink ?>" border="0" alt="<?= $alt_text ?>"></div>
        </td>
        ...
        <td class="list1_base_price" align="right" onclick="window.location.href='<?= $itemlink ?>'">
            <div class="replace_price_string_toggle" data-replacementstring="<?= $replacement_string ?>"
                 data-initstring="<?= $from_price_string ?>"><?= $from_price_string ?></div>
        </td>
    ...
    <div class="itemlist2 itemlist <?=$columns_class?>">
        <a href="<?= $itemlink ?>">
            <div class="itemlist_container">
                ...
                <div class="itemlist_content price base_price">
                    <div class="itemlist_content_label"><?=$GLOBALS['tc']['your_price']?></div>
                     <?
                    if(isset($item['customer_price']) && $item['customer_price'] > 0) {
                        if(isset($item['cross_price']) && $item['cross_price'] > 0 && $item['cross_price'] !== $item['customer_price']) { ?>
                            <div class="itemlist1_cross_price"><?= format_amount($item['cross_price'], false)?></div>
                        <?php
                        }?>
                        <div class="itemlist1_campaign_price"><?= format_amount($item['customer_price'], false)?></div>
                              <?php
    ...
    <div class="itemlist3">
        <div class="itemlist3_top" onclick="window.location.href='<?= $itemlink ?>'">
            <div class="itemlist3_image">
                <div class="itemlist3_image_layer1"><img src="<?= $imagelink ?>" border="0" title="<?= $image['description'] ?>"
...
function show_item_list_4_header($formname) {
    ...
    <form name="<?= $formname ?>" id="<?= $formname ?>"
          action="?action=shop_refresh_user_basket" method="POST">
...
function show_item_list_with_pages( $type, $query_, $category, $vendor_sql_string = "", $max_number = 0, $order_by = "", $show_all_items = false, $is_Search = false, $search_query = "" ) {
    ...
    if (!$is_Search) {
        if ($category != "campain") {
            ...
            $categoryPath = "/campain/?shop_campain=" . $_REQUEST['shop_campain'] . "&";
    ...
    $categoryPath = "/".customizeUrl().$categoryPath;
...
function show_category_list_etsy( $result, $language ) {
	...
	while ($category = @mysqli_fetch_array($result)) {
        ...
        $categorylink = category_get_path($category);
		...
		<div class="categorylist col-xs-6 col-sm-4 col-md-6 col-lg-4">
		    <div class="categorylist_container">
                ...
                <div class="categorylist_content categorylist_images_outer" style="padding-bottom: 10px; padding-top: 10px;">
                    <div class="categorylist_images">
                        <div class="categorylist_images_inner row">
                            <?
                            if(count($items) > 0){
                                $count = "first";
                                for ($i=1; $i<count($items); $i++) {
                                     ...
                                     if ($imageItem["filename"] == '' || !file_exists("../../" . $GLOBALS["shop_setup"]["image_config"][2]["path"] . "/" . $imageItem['filename'])) {
                                        ...
                                        $imagelink = $GLOBALS["shop_setup"]["image_config"][2]["path"] . "/" . $imageItem["filename"];?>
                                        <a href="<?=$itemlink?>" class="categorylist_images_item col-xs-12 col-sm-6 col-md-4 <?= $count ?>">
                            ...
                            ?>
                                    <a href="<?= $categorylink ?>" title="<?=$category['name']?>" class="categorylist_images_item col-xs-12 col-sm-6 col-md-4 categorylist_images_item_counter last">
                                        <div class="categorylist_images_item_inner last">
                                            <div class="image">
                                            <span style="text-align: center; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">+<?= $numberofItems[0]['numer_of_items']?><br><span style="font-size: 14px;">Artikel</span></span>
                                            </div>
                                        </div>
                                    </a>
                            <?
