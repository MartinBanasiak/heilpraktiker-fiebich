...
if (@mysqli_num_rows($result) == 1) {
    ...
    $query             = "SELECT shop_item_id AS 'id', '" . $GLOBALS["shop"]["code"] . "' AS 'shop_code', '" . $GLOBALS["language"]["code"] . "' AS 'language_code',
					 item_no, variant_code,description, summary, allow_invoice_disc, list_price, unit_price, quantity AS 'basket_quantity', line_amount
    ...
    if (!(@mysqli_num_rows(@mysqli_query($GLOBALS['mysql_con'], $query)) > 0)) {
       // echo "no sales lines found. Query: \n $query";
    }
    ?>
    ...
    <table class="mail_bottom" cellpadding="8" cellspacing="0" border="0" width="100%">
        <tr>
            ...
            <td valign="top" align="right" width="50%">
                <? show_order_sum($sales_header["subtotal"], $sales_header["online_discount"], $sales_header["online_discount_amount"], $sales_header["invoice_discount"], $sales_header["invoice_discount_amount"], $sales_header["small_quantity_charge_amount"], $sales_header["total"], $sales_header['shipping_cost'], $sales_header['payment_cost']) ?>
            </td>
        </tr>
    </table>
<?
} else {
  //  echo "no sales header found. Query: \n $query";
}
?>