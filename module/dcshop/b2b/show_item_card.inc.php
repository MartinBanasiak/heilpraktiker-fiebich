...
$result = get_item_by_id($_GET["card"]);
...
if (count($result) > 0) {
    ...
    <div id="itemcard_bottom">
        ...
        if ($hasVariants) {
            ...
            $availabilityProvider = $IOCContainer->create(\DynCom\dc\dcShop\interfaces\ItemAvailabilityProvider::class);
...
EOS;
        ...
        $fit_item_query = "SELECT DISTINCT
        ...
        $alt_item_query = "SELECT DISTINCT
                                shop_view_active_item.*
                            FROM
                                shop_item_link
                            INNER JOIN
        ...
        $spare_part_query = "SELECT DISTINCT
                            ...
                            ORDER BY RAND()
                           LIMIT 6";
        $attributequery = "SELECT shop_attribute_link.*,shop_attribute.description AS 'headline',shop_attribute.data_type,shop_attribute.display_type
                            ,shop_attribute.navision_value,shop_attribute.show_in_header
                          FROM shop_attribute_link
                          INNER JOIN shop_attribute ON shop_attribute.code = shop_attribute_link.attribute_code
                          WHERE shop_attribute_link.company = '" . $GLOBALS['shop']['company'] . "'
                            AND shop_attribute.company = '" . $GLOBALS['shop']['company'] . "'
        ...
        $fit_item_result = mysqli_query($GLOBALS['mysql_con'], $fit_item_query);
        $fit_item_num_rows = mysqli_num_rows($fit_item_result);
        $alt_item_result = mysqli_query($GLOBALS['mysql_con'], $alt_item_query);
        $alt_item_num_rows = mysqli_num_rows($alt_item_result);
        ...
        //echo "# ACC-ITEM [$acc_item_num_rows] - SPARE [$spare_part_num_rows] - DESC [$description_num_rows] ATTR [$attribute_num_rows]";

        if (($description_num_rows + $acc_item_num_rows+ $fit_item_num_rows+ $alt_item_num_rows + $spare_part_num_rows + $number_of_documents) > 0
        ) {
            echo "<div class='tabs'>";
            echo "<ul class=\"tab\">";
            ...
            if ($fit_item_num_rows > 0) {
                $tabontext = ($tab_count == 1) ? " class=\"tabon\"" : " class=\"taboff\"";
                echo "<li id=\"tab" . $tab_count . "\"" . $tabontext . "><a href=\"javascript:void(0);\" onclick=\"showLayer('tab_content" . $tab_count . "');showTab('tab" . $tab_count . "');\">" . $GLOBALS["tc"]["references"] . "</a></li>";
                $tab_count++;
            }
            if ($alt_item_num_rows > 0) {
                $tabontext = ($tab_count == 1) ? " class=\"tabon\"" : " class=\"taboff\"";
                echo "<li id=\"tab" . $tab_count . "\"" . $tabontext . "><a href=\"javascript:void(0);\" onclick=\"showLayer('tab_content" . $tab_count . "');showTab('tab" . $tab_count . "');\">" . $GLOBALS["tc"]["related_items"] . "</a></li>";
                $tab_count++;
            }
            ...
            if ($fit_item_num_rows > 0) {
                $showtext = ($tab_count == 1) ? " class=\"show\"" : " class=\"hide\"";
                echo "<div" . $showtext . " class=\"hide\" id=\"tab_content" . $tab_count . "\">\n";
                echo "<div class='tab_content_headline'>" . $GLOBALS["tc"]["references"] . "</div>";
                show_item_list_from_query($fit_item_query, 1, 'form_itemlist_acc_items');
                echo "</div>\n";
                echo "<div class=\"clearfloat\"></div>\n";
                //echo "<a id=\"_tab_content" . $tab_count . "\" name=\"_tab_content" . $tab_count . "\"></a>\n";
                $tab_count++;
            ...
            if ($alt_item_num_rows > 0) {
                $showtext = ($tab_count == 1) ? " class=\"show\"" : " class=\"hide\"";
                echo "<div" . $showtext . " class=\"hide\" id=\"tab_content" . $tab_count . "\">\n";
                echo "<div class='tab_content_headline'>" . $GLOBALS["tc"]["related_items"] . "</div>";
                show_item_list_from_query($alt_item_query, 1, 'form_itemlist_acc_items');
                echo "</div>\n";
                echo "<div class=\"clearfloat\"></div>\n";
                //echo "<a id=\"_tab_content" . $tab_count . "\" name=\"_tab_content" . $tab_count . "\"></a>\n";
                $tab_count++;
        ...
        <div class="item_main_image">
            <div class="item_main_container">
                <a href="<?= $image_config[4]["path"]; ?>/<?= $main_image["filename"]; ?>" class="MagicZoomPlus"
                   ...
                   hint: off;
                   cssClass: white-bg; ">
                    <img id="item_main_picture" src="<?= $image_config[3]["path"]; ?>/<?= $main_image["filename"]; ?>"
                         alt="<?= $main_image["description"]; ?>" title="<?= $main_image["title"]; ?>" />
                </a>
            </div>
        </div>
                        ...
                        <a href="<?= $image_config[4]["path"] . "/" . $tumb_image["filename"] ?>" data-zoom-id="zoom"
                           data-image="<?= $image_config[3]["path"] . "/" . $tumb_image["filename"] ?>">
                            <img src="<?= $image_config[1]["path"] . "/" . $tumb_image["filename"]; ?>"
                                 alt="<?= $tumb_image["description"]; ?>"
                                 title="<?= $tumb_image["description"]; ?>"/>
                        </a>
                        <?
                    }
                }
...
function variant_select_nav($item)
{
    ...
    if (mysqli_num_rows($var_result) > 0) {
        ...
        <select class='select' name='input_variant'
                onchange="location.href='-p<?= $_GET['card'] ?>/?var=true&variant='+this.options[this.selectedIndex].value;">
            <?
