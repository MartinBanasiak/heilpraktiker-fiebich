...
$title['de'] = "Kategorebeschreibung bearbeiten";
$title['en'] = "Edit category description";
?>
<head>
...
$countquery = "SELECT id,category_description,category_description_2,promotion_description FROM shop_category WHERE company = '" . $_GET["company"] . "' AND shop_code = '" . $_GET["shop_code"] . "' AND language_code = '" . $_GET["language_code"] . "' AND line_no = '" . $_GET["line_no"] . "'";
$result = @mysqli_query($GLOBALS['mysql_con'],$countquery);
if(@mysqli_num_rows($result)==1) {
	$category = @mysqli_fetch_assoc($result);
	switch($_GET["type"]) {
		case 0: $content_field = "category_description"; break;
		case 1:	$content_field = "promotion_description"; break;
		case 2:	$content_field = "category_description_2"; break;
	}
	// Änderungen speichern
	if($_GET["send"]) {
		$query = "UPDATE shop_category SET " . $content_field . " = '" . $_POST["textcontent"] . "' WHERE id = '" . $category["id"] . "' LIMIT 1";
...
<? 
$editor_style = ($_GET["rtc"]=="TRUE") ? "nav_rtc" : "nav";
...
show_ck_editor($category[$content_field],"nav",$GLOBALS["shop_setup"]["fck_style_dir"],$GLOBALS["shop_setup"]["fck_xml_dir"]); 
?>
...
<?
	...
	echo "Fehler in Webform 'edit_category_description'. \nquery: " . $countquery . "\n";
}
