...
$title['de'] = "Platzhalterbild bearbeiten";
$title['en'] = "Edit placeholder image";
...
$countquery = "SELECT id, item_placeholder_image FROM shop_language WHERE company = '" . $_GET["company"] . "' AND shop_code = '" . $_GET["shop_code"] . "' AND code = '" . $_GET["language_code"] . "'";
	...
	if(isset($_FILES['input_file'])) {
        ...
        if(is_valid_image($_FILES['input_file'])){
			foreach($GLOBALS["shop_setup"]["image_config"] as $image) {
				$oldfile = "../../.." . $image["path"]  ."/". $language["item_placeholder_image"];
				if(file_exists($oldfile)) {
					@unlink($oldfile);
				}
			}
			$language["item_placeholder_image"] = $language["id"] . "_" . filename_validation($_FILES['input_file']['name']);
			$uploadfile = "../../.." . $GLOBALS["shop_setup"]["uploaddir"]  ."/". $language["item_placeholder_image"];
			if(file_exists($uploadfile)) {
				@unlink($uploadfile);
			}
			if(move_uploaded_file($_FILES['input_file']['tmp_name'],$uploadfile)) {
				foreach($GLOBALS["shop_setup"]["image_config"] as $image) {
					$newuploadfile = "../../.." . $image["path"]  ."/". $language["item_placeholder_image"];
					image_resize($uploadfile,$newuploadfile,$image["maxwidth"],$image["maxheight"]);
				}
				$newuploadfile = "../../../" . $GLOBALS["shop_setup"]["uploaddir_category_icon"] ."/". $language["item_placeholder_image"];
				image_resize($uploadfile,$newuploadfile,$GLOBALS["shop_setup"]["category_icon_maxwidth"],$GLOBALS["shop_setup"]["category_icon_maxheight"]);
				@unlink($uploadfile);
				$query = "UPDATE shop_language SET item_placeholder_image = '" . $language["item_placeholder_image"] . "' WHERE id = " . $language["id"];
				if (!@mysqli_query($GLOBALS['mysql_con'],$query)) {
					echo "Fehler in Webform 'edit_item_placeholder_image'. query: " . $query . "\n";
					exit();
				}
			} else {
				echo "Fehler in Webform 'edit_item_placeholder_image'. query: " . $countquery . "\n";
				exit();
			}
		} else {
...
<div id="box1">
<?
if ($language["item_placeholder_image"] <> '') {
	// Bild anzeigen
	echo "<img style=\"max-width: 100%; max-height: 100%;\" src=\"" . $GLOBALS["shop_setup"]["image_config"][3]["path"] . "/". $language["item_placeholder_image"] . "?id=" . rand(1,900) . "\" />\n";
} else {
	echo $file_message;
}
?>
...
<?
	...
	echo "Fehler in Webform 'edit_item_placeholder_image'. \nquery: " . $countquery . "\n";
}
