...
$file_message2['de'] = "Bitte wählen Sie ein Video im Flash-Videoformat (*.flv)\n";
$file_message2['en'] = "Choose a video in flash-format(*.flv)\n";
...
$countquery = "SELECT * FROM shop_item_file WHERE company = '" . $_GET["company"] . "' AND shop_code = '" . $_GET["shop_code"] . "' AND language_code = '" . $_GET["language_code"] . "' AND item_no = '" . $_GET["item_no"] . "' AND type = '" . $_GET["type"] . "' AND line_no = '" . $_GET["line_no"] . "'";
...
if(@mysqli_num_rows($result)==1) {
	...
	switch($input_file["type"]) {
		...
		case 0:
			...
			if(isset($_FILES['input_file'])) {
                ...
                if(is_valid_image($_FILES['input_file'])){
					...
					$input_file["filename"] = $input_file["id"] . "_" . filename_validation($_FILES['input_file']['name']);
					$uploadfile = "../../.." . $GLOBALS["shop_setup"]["uploaddir"] . $input_file["filename"];
					...
					if(move_uploaded_file($_FILES['input_file']['tmp_name'],$uploadfile)) {
						...
						@unlink($uploadfile);
						$query = "UPDATE shop_item_file SET filename = '" . $input_file["filename"] . "' WHERE id = " . $input_file["id"];
						if (!@mysqli_query($GLOBALS['mysql_con'],$query)) {
							echo "Fehler in Webform 'edit_file'. query: " . $query . "\n";
							exit();
						}
					} else {
						echo "Fehler in Webform 'edit_file'. query: " . $countquery . "\n";
						exit();
					}
				} else {
					$input_file["filename"] = '';
		...
		case 1:
			...
			if(isset($_FILES['input_file'])) {
				$oldfile = "../../.." . $GLOBALS["shop_setup"]["uploaddir_documents"] . $input_file["filename"];
				if(file_exists($oldfile)) {
					@unlink($oldfile);
				}
				$input_file["filename"] = $input_file["id"] . "_" . filename_validation($_FILES['input_file']['name']);
				$uploadfile = "../../.." . $GLOBALS["shop_setup"]["uploaddir_documents"] . $input_file["filename"];
				if(file_exists($uploadfile)) {
					@unlink($uploadfile);
				}
				if(move_uploaded_file($_FILES['input_file']['tmp_name'],$uploadfile)) {
		...
		// Videos Speichern
		case 2:
			$file_message = "<br /><br />".$file_message2[$language_code];
			if(isset($_FILES['input_file'])) {
				if(preg_match('/\.FLV$/',strtoupper($_FILES['input_file']['name']))) {
					$oldfile = "../../.." . $GLOBALS["shop_setup"]["uploaddir_videos"] . $input_file["filename"];
					if(file_exists($oldfile)) {
						@unlink($oldfile);
					}
					$input_file["filename"] = $input_file["id"] . "_" . filename_validation($_FILES['input_file']['name']);
					$uploadfile = "../../.." . $GLOBALS["shop_setup"]["uploaddir_videos"] . $input_file["filename"];
					if(file_exists($uploadfile)) {
						@unlink($uploadfile);
					}
					if(move_uploaded_file($_FILES['input_file']['tmp_name'],$uploadfile)) {
...
if ($input_file["filename"] <> '') {
	switch($input_file["type"]) {
		// Bild anzeigen
		case 0:
			echo "<img style=\"max-width: 100%; max-height: 100%;\" src=\"" . $GLOBALS["shop_setup"]["image_config"][3]["path"] ."/". $input_file["filename"] . "?id=" . rand(1,900) . "\" />\n";
		break;
		// Datei anzeigen (als Symbol)
		case 1:
			$button_class = get_button_file_typ($input_file["filename"]);
			if (strpos($input_file["filename"],'_')) {
				$input_file["filename"] = substr($input_file["filename"],strpos($input_file["filename"],'_')+1);
			}
			echo "<br /><br /><br /><br /><ul class=\"buttonlist\"><li><a class=\"" . $button_class . "\" href=\"/module/dcshop/webforms/download_file.php?file=" . $input_file["id"] ."\">" . urldecode($input_file["filename"]) . "</a></li></ul>\n";
		break;
		// Video in Flashplayer anzeigen
		case 2:
			$flash_file = $GLOBALS["shop_setup"]["uploaddir_videos"] . $input_file["filename"];
			$flash_width = "300";
			$flash_height = "250";
			require $rootDir . DIRECTORY_SEPARATOR . 'plugins/flashplayer/flashplayer.inc.php';
		break;
	}
} else {
	echo $file_message;
...
<?
	...
	echo "Fehler in Webform 'edit_file'. \nquery: " . $countquery . "\n";
}
