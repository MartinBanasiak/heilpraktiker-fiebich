...
class PomSoapClient extends SoapClient {
    ...
    protected function parseHeaders( $headers ) {
        ...
        if (function_exists('http_parse_headers')) {
            ...
            if (!isset($parsedHeaders['Set-Cookie']) || is_array($parsedHeaders['Set-Cookie'])) {
        ...
        foreach ($headers as $header) {
            ...
            if (!isset($elements[0]) || strlen($elements[0]) < 1 || $elements[0][strlen($elements[0]) - 1] != ':') {
                continue;
            }
    ...
    public function __call( $methodname, $params ) {
        ...
        if (!is_array($params)) {
            throw new Exception("Invalid parameters, array is required");
        }
        ...
        foreach ($params as $key => $value) {
            ...
            if ($this->isAssoc($value)) {
                $params[$key] = $this->encodeAssoc($value);
            } elseif (is_array($value)) {
                $params[$key] = $this->encodeArray($value);
            } elseif (is_object($value)) {
                $params[$key] = $this->encodeObject($value);
            } else {
                $params[$key] = $this->convertToApi($value);
            }
        }
    ...
    private function encodeAssoc( $array ) {
        ...
        foreach ($array as $key => $value) {
            ...
            if (!is_int($key) && !is_string($key)) {
                throw new Exception('Invalid parameter: Complex keys are not supported.');
                continue;
            }

            // check if we have a proper value.
            if (is_null($value)) {
                throw new Exception('Invalid parameter: NULL values are not supported.');
                continue;
            }

            // we do not support nested objects for parameters
            if (is_object($value)) {
                throw new Exception('Invalid parameter: Nested objects are not supported.');
                continue;
            }

            // we want to check if someone is trying to make a nested array inside our assoc
            if ($this->isAssoc($value)) {
                throw new Exception('Invalid parameter: Nested associative arrays are not supported');
            }

            // only when a string, it has to be encoded
            if (is_string($key)) {
                $key = $this->convertToApi($key);
            }
            if (is_string($value)) {
                $value = $this->convertToApi($value);
            }
    ...
    private function encodeArray( $array ) {
        ...
        foreach ($array as $value) {
            ...
            if (is_object($value)) {
                ...
                throw new Exception('Invalid parameter: arrays of objects are not supported');
            } elseif (is_string($value)) {
                $result[] = $this->convertToApi($value);
            } else {
                $result[] = $value;
            }
        }
    ...
    private function decodeResult( $result ) {
        ...
        if (isset($result->map)) {
            ...
            foreach ($pairs as $pair) {
                $result[$this->convertFromApi($pair->key)] = $this->convertFromApi($pair->value);
            }
            return $result;
        }
        ...
        if (isset($result->value)) {
            return is_string($result->value) ? $this->convertFromApi($result->value) : $result->value;
        }
    ...
    private function decodeObject( $object ) {
        ...
        foreach (get_object_vars($object) as $key => $value) {
            ...
            if (is_string($key)) {
                $key = $this->convertFromApi($key);
            }
            if (is_string($value)) {
                $value = $this->convertFromApi($value);
            }
            if (is_object($value)) {
                $value = $this->decodeObject($value);
            }
