...
class TrackingEventVisitorCategoryCounterUpdater implements PipelineStage
{
    ...
    const VISITOR_CATEGORY_UPDATE_QUERY = '
        INSERT INTO 
          visitor_category
        SET 
          visitor_id = :visitor_id,
          category_id = :category_id,
          view_duration_seconds            = :additional_view_seconds,
          view_counter                     = :additional_views,
          item_add_to_basket_counter       = :additional_basket_additions,
          item_total_add_to_basket_qty     = :additional_basket_add_qty,
          item_total_add_to_basket_value   = :additional_basket_add_value,
          item_rm_from_basket_counter      = :additional_basket_removals, 
          item_total_rm_from_basket_qty    = :additional_basket_rm_qty, 
          item_total_rm_from_basket_value  = :additional_basket_rm_value,
          item_order_counter               = :additional_item_order_count, 
          item_total_order_qty             = :additional_item_order_qty,
          item_total_order_value           = :additional_item_order_value
    ...
    protected function updateVisitorCategory(
        $visitorID,
        $categoryID,
        ...
        $stmt = $this->trackingDBHandler->prepare(self::VISITOR_CATEGORY_UPDATE_QUERY);
        $stmt->bindValue(':' . self::KEY_VISITOR_ID, (int)$visitorID, \PDO::PARAM_INT);
    ...
    protected function handlePageviewCompleteEvent(TrackingEvent $trackingEvent)
    {
        ...
        if ($referencedEvent) {
            ...
            $userID = isset($referencedEventData[self::KEY_CURRENT_USER_ID]) ? $referencedEventData[self::KEY_CURRENT_USER_ID] : null;
            $customerID = isset($referencedEventData[self::KEY_CURRENT_CUSTOMER_ID]) ? $referencedEventData[self::KEY_CURRENT_CUSTOMER_ID] : null;
            ...
            if ($visitorID && $categoryID && $additionalViewSeconds) {
                $this->updateVisitor($visitorID, $additionalViewSeconds, 0);
                $this->updateVisitorCategory($visitorID, $categoryID, $additionalViewSeconds);
            }
        }
    ...
    protected function handlePageviewCategory(TrackingEvent $trackingEvent)
    {
        ...
        if ($visitorID && $categoryID) {
            $this->updateVisitor($visitorID, 0, $additionalViews);
            $this->updateVisitorCategory($visitorID, $categoryID, 0, $additionalViews);
        }
    }
    ...
    protected function handleOrderCompleteEvent(TrackingEvent $trackingEvent)
    {
        ...
        foreach ($valuesPerCategory as $categoryID => $valueArr) {
            ...
            if ($visitorID) {
                $this->updateVisitorCategory($visitorID, $categoryID, 0, 0, 0, 0, 0, 0, 0, 0, $no, $qty, $value);
            }
