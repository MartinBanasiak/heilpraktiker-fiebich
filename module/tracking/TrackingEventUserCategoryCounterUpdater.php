...
class TrackingEventUserCategoryCounterUpdater implements PipelineStage
{
    ...
    const USER_CATEGORY_UPDATE_QUERY = '
        INSERT INTO 
          user_category
        SET 
          user_id = :user_id,
    ...
    protected function updateUser($userID, $additionalViewSeconds, $additionalViews)
    {
        ...
        $stmt->bindValue(':' . self::KEY_USER_ID, (int)$userID, \PDO::PARAM_INT);
    ...
    protected function updateUserCategory(
        $userID,
        $categoryID,
        ...
        $stmt = $this->trackingDBHandler->prepare(self::USER_CATEGORY_UPDATE_QUERY);
        $stmt->bindValue(':' . self::KEY_USER_ID . '', (int)$userID, \PDO::PARAM_INT);
    ...
    protected function handlePageviewCompleteEvent(TrackingEvent $trackingEvent)
    {
        ...
        if ($referencedEvent) {
            ...
            $eventDataUserID = isset($eventData[self::KEY_CURRENT_USER_ID]) ? (int)$eventData[self::KEY_CURRENT_USER_ID] : null;
            $referencedEventUserID = $referencedEvent->getUserID();
            $referencedEventDataUserID = isset($referencedEventData[self::KEY_CURRENT_USER_ID]) ? (int)$referencedEventData[self::KEY_CURRENT_USER_ID] : null;
            $userID = null;
            if ($eventUserID) {
                $userID = $eventUserID;
            ...
            if ($userID && $categoryID && $additionalViewSeconds) {
                $this->updateUser($userID, $additionalViewSeconds, 0);
                $this->updateUserCategory($userID, $categoryID, $additionalViewSeconds);
            }
        }
    ...
    protected function handlePageviewCategory(TrackingEvent $trackingEvent)
    {
        ...
        if ($userID && $categoryID) {
            $this->updateUser($userID, 0, $additionalViews);
            $this->updateUserCategory($userID, $categoryID, 0, $additionalViews);
        }
    }
    ...
    protected function handleOrderCompleteEvent(TrackingEvent $trackingEvent)
    {
        ...
        foreach ($valuesPerCategory as $categoryID => $valueArr) {
            ...
            if ($userID) {
                $this->updateUserCategory($userID, $categoryID, 0, 0, 0, 0, 0, 0, 0, 0, $no, $qty, $value);
            }
        }
    }
