...
class TrackingEventCategoryCounterUpdater implements PipelineStage
{

    const CATEGORY_UPDATE_QUERY = '
        UPDATE 
          category
        SET 
    ...
    protected function updateCategory(
        ...
        $stmt = $this->trackingDBHandler->prepare(self::CATEGORY_UPDATE_QUERY);
    ...
    protected function handlePageviewCompleteEvent(TrackingEvent $trackingEvent)
    {
        ...
        if ($referencedEvent) {
            ...
            if ($categoryID && $additionalViewSeconds) {
                $this->updateCategory($categoryID, $additionalViewSeconds);
            }
        }
    ...
    protected function handlePageviewCategory(TrackingEvent $trackingEvent)
    {
        ...
        if ($categoryID) {
            $this->updateCategory($categoryID, 0, $additionalViews);
        }
    }
    ...
    protected function handleOrderCompleteEvent(TrackingEvent $trackingEvent)
    {
        ...
        foreach ($valuesPerCategory as $categoryID => $valueArr) {
            ...
            $this->updateCategory($categoryID, 0, 0, 0, 0, 0, 0, 0, 0, $no, $qty, $value);
        }
    }
