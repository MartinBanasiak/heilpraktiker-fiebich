 ...
 */
class CustomerTopItemsQueueWorker implements AutonomousQueueWorker
{
     ...
     */
    public function __construct(JobQueueGateway $gateway, CustomerTopItemsJobHandler $jobHandler)
    {
...
$logFilePath = $baseDir . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . 'CustomerTopItems.log';
...
$logger = new Logger('CustomerTopItems', [$logFileHandler]);
...
$envDirWorker = $envDirWorkerqueue . DIRECTORY_SEPARATOR . 'workers' . DIRECTORY_SEPARATOR . 'customerTopItems';
...
$handlerDBHost = getenv('CUSTOMER_ITEM_DB_HOST');
$handlerDBPort = getenv('CUSTOMER_ITEM_DB_PORT');
$handlerDBUser = getenv('CUSTOMER_ITEM_DB_USER');
$handlerDBPass = getenv('CUSTOMER_ITEM_DB_PASS');
$handlerDBSchema = getenv('CUSTOMER_ITEM_DB_SCHEMA');
$handlerDSN = 'mysql:dbname=' . $handlerDBSchema . ';host=' . $handlerDBHost . ';port=' . (int)$handlerDBPort . ';charset=utf8mb4';
$handler = new CustomerTopItemsJobHandler($handlerDSN, $handlerDBUser, $handlerDBPass);
$handler->setLogger($logger);

ini_set('display_errors', '1');
error_reporting(E_ERROR);

$workerInstance = new CustomerTopItemsQueueWorker($gateway, $handler);
